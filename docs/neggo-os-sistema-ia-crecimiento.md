...
| **Acceso** | `estrategia-adquisicion-clientes-neggo.md`, **CRM Ventas real (tabla `crm_ventas_leads` en Supabase, admin de Neggo — reemplazó el Excel el 10 ago; ver `docs/spec-crm-ventas-admin.md`)**, Instagram/Maps/LinkedIn (vía Claude in Chrome, sesión de Jhey), taxonomía de categorías de `negocio-me-interesa-metas.md`. Escribe leads nuevos vía `crm_ventas_crear_lead` — nunca INSERT/UPDATE directo. |
...
| **Acceso** | Guion de llamada y objeciones (`estrategia-adquisicion-clientes-neggo.md` secciones 4 y 6), **CRM Ventas real (tabla `crm_ventas_leads` en Supabase — reemplazó el Excel el 10 ago; ver `docs/spec-crm-ventas-admin.md`)**. Genera la respuesta sugerida vía `crm_ventas_guardar_respuesta_sugerida` — nunca INSERT/UPDATE directo. Marcar "Enviado", registrar la respuesta real de un negocio, y mover la etapa (Agendado/Cerrado/Perdido) son SIEMPRE acciones manuales de Jhey desde el panel — ninguna función que las ejecuta permite automatización, a propósito. |
