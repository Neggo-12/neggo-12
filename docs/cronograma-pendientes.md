...
1. npm audit — 2 vulnerabilidades restantes (paso a ya resuelto), ninguna con exploit activo conocido en producción hoy. Plan de ejecución: una por sesión, de menor a mayor riesgo:
   ...
   - ~~b. esbuild/vite → vite 8~~ — **hecho, pendiente de validación final (25 jul)**. Vite 8 cambió de bundler interno a Rolldown — nuestro `vite.config.ts` es mínimo (sin `rollupOptions` ni hooks de plugin custom), así que no necesitó cambios. Se detectó y corrigió de paso una incompatibilidad real: `@vitejs/plugin-react-swc@3.11.0` solo soporta hasta vite 7 (peer dep), bumpeado a `4.3.2`. `vitest` también bumpeado 3.2.4→4.1.10 en el mismo paso porque vitest 3 no declaraba vite como peer explícito y su soporte real de vite 8 no está garantizado — el repo no tiene `vitest.config.ts` ni usa ninguna de las opciones removidas en v4 (poolOptions, workspace), así que el riesgo de config rota es bajo. tsc, lint y build limpios en sandbox; falta la corrida real de `vitest` (bug arm64) vía Claude Code antes de dar esto por cerrado.
