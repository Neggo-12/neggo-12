...
- **"Garantía Neggo" — programa de garantía para talleres de Carro/Moto (agregado 10 ago, propuesta de Jhey):** en vez de que Neggo garantice reparaciones sin fondo real (riesgo directo de terminar pagando daños ajenos), el taller se afilia a un programa y aporta a un fondo — cuota mensual o % del valor de cada reparación (ej. 1-3%). Si el taller no responde por una garantía válida, Neggo paga al cliente desde el fondo y luego cobra al taller o lo descuenta de pagos futuros. Niveles propuestos: Básico (Neggo solo media el conflicto, sin fondo) / Garantía Neggo (aporta al fondo, cliente tiene respaldo económico) / Premium (inspecciones periódicas, mayor cobertura, prioridad en el listado). Incluye mecánica de deducible (ej. taller asume los primeros $200.000 de una reparación de $800.000, el fondo cubre el resto) para reducir fraude y que el taller cuide la calidad. Alternativa que Jhey también propuso: en vez de que Neggo administre el fondo directamente, aliarse con una aseguradora real — el taller compra una micropóliza colectiva y Neggo recibe comisión por póliza vendida, transfiriendo el riesgo actuarial a un tercero especializado.
  - **Por qué tiene sentido estratégicamente:** encaja directo con el diferenciador ya validado de Neggo (confianza/anti-fraude, ver `marketing-neggo.md` sección 3) y con el problema real que documenta `docs/estudio-estafas-internet-2025-2026.md` — el freno de conversión más grande es la desconfianza, y hoy el Sello de Confianza es una verificación (sí/no), no un respaldo económico. Un mensaje tipo "si el taller no responde, tenés respaldo económico" es un salto real de diferenciación frente a cualquier directorio de talleres. Carro y Moto ya son categorías reales de Neggo (`ComercioCategory`/`GoalCategory`), no hay que crear taxonomía nueva.
  - **Por qué NO se puede avanzar solo desde Marketing/CGO:** administrar un fondo de garantía (aunque sea pequeño) y/o intermediar pólizas de seguro son actividades reguladas en Colombia por la Superintendencia Financiera — captación de dineros del público y correduría/agencia de seguros típicamente requieren autorización específica. Esto no es una opinión legal (no soy abogado ni asesor financiero, y esto no lo reemplaza) — es una señal de que antes de construir o de comunicar esto como beneficio disponible, hace falta: (1) validación legal real sobre qué figura aplica (fondo propio vs. alianza con aseguradora) y qué requiere licencia, (2) diseño técnico con `neggo-architect` (ledger del fondo, flujo de reclamación, tracking de deducibles — no es una tabla más, es dinero real con reglas de disputa), (3) `neggo-security` para el modelo de abuso/fraude del propio fondo (reclamaciones falsas).
  - **Estado:** idea documentada, no evaluada técnica ni legalmente todavía. No comunicar como beneficio real en ningún contenido/propuesta hasta que se resuelvan los tres puntos de arriba — rompería la regla de cero beneficios inventados si se promete antes de tiempo.
...
## Completado (sesión 25 jul 2026, continuación 3) — Auto-reload en fallo de chunk
`ErrorBoundary.tsx` detecta el mensaje "Failed to fetch dynamically imported module" (13/23 de los fallos reales en `fallos_app`) y recarga la página una sola vez en vez de mostrar "Algo salió mal" — con guard de `sessionStorage` para no entrar en loop si el error persiste por otra razón. Mismo tratamiento en `main.tsx` para el evento nativo `vite:preloadError`. Sigue registrando en `fallos_app`/Sentry antes de recargar. Commit `1c162b5` — pendiente de deploy.
